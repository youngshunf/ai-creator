...
依赖方向（单向）:
  ...
  backend → agent-core
  tauri-app → python-sidecar (IPC)
  tauri-app → backend (HTTP API)

禁止:
  - agent-core 依赖 python-sidecar 或 backend
  - 循环依赖
  - 跨层直接调用（必须通过接口）

共享代码:
  位置: agent-core
  原则: 端云共用的代码必须放在 agent-core
```
