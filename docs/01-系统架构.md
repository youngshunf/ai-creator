...
- **browser-use AI浏览器**: AI Agent 直接操作虚拟浏览器完成发布，无需编写选择器和流程脚本
...
|------|--------|---------|---------|-----------|
| **uni-app** ✅ | iOS + Android + 小程序 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| Flutter | iOS + Android | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| React Native | iOS + Android | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| Tauri Mobile | iOS + Android | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ |

**移动端选择**: **uni-app (TypeScript) + unibest**

**选择理由**:

1. 国内生态成熟：插件丰富，文档完善
2. 多端发布：一套代码同时发布 iOS/Android/微信小程序
3. unibest 最佳实践：开箱即用的工程化配置
4. 纯云端模式：移动端不需要本地能力，uni-app 完全满足
5. 开发效率：Vue 3 + TypeScript，上手快
...
浏览器服务:
  - browser-use: AI Agent 操作虚拟浏览器（无需选择器/流程脚本）
...
│   │       └── services/                # 本地服务
│   │
│   ├── mobile-app/                      # uni-app x 移动端 (基于 unibest)
│   │   ├── src/                         # 源码目录
│   │   │   ├── pages/                   # 页面
│   │   │   ├── components/              # 组件
│   │   │   └── stores/                  # 状态管理
...
│  │  │ │use + AI  │ │ │use + AI  │ ││use+AI  │ │ │use + AI  │ │ │use + AI  │ │            │ │
...
| 能力 | 桌面端 (Tauri) | 移动端 (uni-app) |
|------|---------------|-------------------|
| **技术栈** | Tauri 2.0 + React + Rust | uni-app + Vue 3 + TypeScript (unibest) |
| **项目位置** | apps/desktop (Monorepo) | apps/mobile-app (Monorepo) |
| **Python Sidecar** | ✅ 本地运行 | ❌ 不支持 |
| **AI浏览器操作** | ✅ 本地 browser-use | ☁️ 云端执行 |
| **AI Agent** | ✅ 本地/云端可选 | ☁️ 仅云端 |
| **定时任务** | ✅ 本地APScheduler | ☁️ 云端调度 |
| **数据存储** | ✅ 本地SQLite | ✅ 本地 + 云端同步 |
| **素材采集** | ✅ 文件系统 | ✅ 相机/相册/录音 |
| **离线使用** | ✅ 完整功能 | ⚠️ 仅草稿/缓存 |
| **发布平台** | Windows/macOS/Linux | iOS/Android/小程序 |
...
│ (Vue + TS)  │   REST + SSE   │  (FastAPI)   │                │  (Runtime)  │
└─────────────┘                └──────┬──────┘                └──────┬──────┘
       ▲                              │                               │
       │                              │ 事件推送                       ▼
       │         ┌────────────────────┴────────────────────┐   ┌─────────────┐
       └─────────│ SSE 事件流                              │   │ Tool Registry│
                 │ - node_started                          │   │ browser-use │
                 │ - tool_called                           │   │ (AI浏览器)  │
                 │ - run_completed                         │   └─────────────┘
                 └─────────────────────────────────────────┘
```
...
AI浏览器特性:
  操作方式: AI Agent 直接理解页面内容并操作
  无需选择器: 不依赖 CSS/XPath 选择器
  自适应能力: 自动适应平台 UI 变化
  智能重试: AI 自主判断操作结果并重试
```
