...
import { useProjectStore, ProjectCreate, ProjectUpdate } from '@/stores/useProjectStore';
...
// 行业选项
const INDUSTRIES = [
    '科技', '美妆', '时尚', '美食', '旅游', '教育', '健身', '金融',
    '游戏', '影视', '音乐', '摄影', '生活方式', '母婴', '宠物', '其他',
];
...
function ProjectSettingsPage() {
    ...
    // 添加 tag
    const addTag = (type: 'topics' | 'keywords' | 'brand_keywords', value: string) => {
    ...
    // 移除 tag
    const removeTag = (type: 'topics' | 'keywords' | 'brand_keywords', index: number) => {
        setFormData((prev) => ({
            ...prev,
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
                {/* 项目列表 */}
                <div className={cn('w-80 overflow-y-auto space-y-2', isEditing && 'hidden md:block')}>
                    {projects.length === 0 ? (
                        ...
                        <div className="max-w-2xl space-y-6">
                            ...
                            <div className="p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-2 block">品牌调性</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-2 block">品牌关键词</label>
                                        <div className="flex flex-wrap gap-2 mb-2">
                                            {formData.brand_keywords?.map((kw, i) => (
                                                <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-sm">
                                                    {kw}
                                                    <button onClick={() => removeTag('brand_keywords', i)} className="hover:text-red-500">
                                                        <X className="w-3 h-3" />
                                                    </button>
                                                </span>
                                            ))}
                            ...
                            <div className="p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {formData.topics?.map((topic, i) => (
                                        <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded text-sm">
                                            {topic}
                                            <button onClick={() => removeTag('topics', i)} className="hover:text-red-500">
                                                <X className="w-3 h-3" />
                                            </button>
                                        </span>
                                    ))}
                            ...
                            <div className="flex items-center justify-between pt-4">
                                <div>
                                    {editingProject && !projects.find(p => p.id === editingProject)?.is_default && (
                                        <button
