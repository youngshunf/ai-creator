...
export const usePublishStore = create<PublishState>((set, get) => ({
  ...
  fetchTasks: async (contentId: string) => {
    ...
    try {
      const tasks = await invoke<PublishTask[]>("db_list_publications", {
        contentId,
      });
  ...
  createTask: async (contentId, accountId, platform, scheduledAt) => {
    ...
    try {
      ...
      const userId = authUser?.uuid ?? "current-user";
    ...
    try {
      ...
      for (const account of accounts) {
        ...
        setTimeout(async () => {
          ...
          await get().updateTaskStatus(
            task.id,
            success ? "success" : "failed",
            success ? undefined : "发布失败，请重试",
          );
        }, 3000);
      }
      set({ loading: false });
  ...
  updateTaskStatus: async (taskId, status, errorMessage) => {
    try {
      await invoke("db_update_publication_status", {
        id: taskId,
        status,
        errorMessage,
      });
