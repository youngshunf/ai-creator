...
import { useAccountStore, Platform, PlatformInfo } from '@/stores/useAccountStore';
...
export function AddAccountDialog({ open, onClose, projectId }: AddAccountDialogProps) {
  const { platforms, addAccount } = useAccountStore();
  ...
  const handleSelectPlatform = async (platform: PlatformInfo) => {
    ...
    try {
      ...
      const result = await invoke<{ status: string; account_id: string; message: string }>('start_platform_login', {
      ...
      pollingRef.current = setInterval(async () => {
        console.log('[LOGIN] 轮询检查登录状态, platform:', platform.name, 'accountId:', accountIdRef.current);
        try {
          ...
          if (status.status === 'success' && status.logged_in) {
            ...
            await addAccount(
              ...
              accountName,
              { avatar_url: status.avatar_url, followers_count: status.followers_count }
            );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md p-6">
        <button onClick={handleClose} className="absolute top-4 right-4 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-lg">
          <X className="w-5 h-5" />
        </button>

        <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">
          {step === 'select' ? '选择平台' : `登录 ${selectedPlatform?.display_name}`}
        </h2>

        {step === 'select' ? (
          <div className="space-y-3">
            {platforms.map((platform) => {
              const icon = platformIcons[platform.name] || { bg: 'bg-slate-500', text: '?' };
              return (
                <button
                  key={platform.name}
                ...
                >
                  <div className={`w-10 h-10 rounded-full ${icon.bg} flex items-center justify-center text-white font-bold text-sm`}>
                    {icon.text}
                  </div>
                  <div className="text-left">
                    <div className="font-medium text-slate-900 dark:text-white">{platform.display_name}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">
                      标题 {platform.spec.title_max_length} 字 · 正文 {platform.spec.content_max_length} 字 · {platform.spec.image_max_count} 张图
                    </div>
                  </div>
                </button>
              );
          ...
          <div className="text-center py-8">
            {isLoading ? (
              ...
              <div className={`w-16 h-16 rounded-full ${platformIcons[selectedPlatform?.name || '']?.bg || 'bg-slate-500'} flex items-center justify-center text-white font-bold text-2xl mx-auto mb-4`}>
                {platformIcons[selectedPlatform?.name || '']?.text || '?'}
              </div>
            )}
            <p className="text-slate-600 dark:text-slate-300">{loginStatus}</p>
            <button onClick={() => setStep('select')} className="mt-4 text-sm text-primary-500 hover:text-primary-600">
              返回选择平台
            </button>
          </div>
        )}
